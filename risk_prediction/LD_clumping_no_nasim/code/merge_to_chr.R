...
# Goal: merge the imputed gen format data of BCAC by chr for icogs and onco array
# all onco array data is saved as  /data/BCAC/impute_onco/onco_all.gen.gz; by chromosome data is removed to save space
# all icogs data is saved as  /data/BCAC/impute_icogs/icog_all.gen.gz; by chromosome data is removed to save space
# Author: Haoyu Zhang
#-------------------------------------------------------------------
...
write.table(merge.to.chr,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/LD_clumping/code/merge.to.chr.onco.sh"),col.names = F,row.names = F,quote=F)
...
#write out the command and submit use cluster
write.table(merge.to.chr,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/LD_clumping/code/merge.to.chr.icog.sh"),col.names = F,row.names = F,quote=F)
