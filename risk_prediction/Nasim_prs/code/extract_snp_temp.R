...
colnames(extract_snp) <- "SNP.ONCO"
write.table(extract_snp,file = paste0("./risk_prediction/Nasim_prs/result/extract_snp_temp_onco.txt"),row.names=F,col.names=T,quote=F)
...
for(i in 1:1){
  ...
  temp <- paste0("/data/zhangh24/qctool_v1.4-linux-x86_64/qctool -g ",Files[i]," -incl-rsids /data/zhangh24/breast_cancer_data_analysis/risk_prediction/Nasim_prs/result/extract_snp_temp_onco.txt -og /data/zhangh24/breast_cancer_data_analysis/risk_prediction/Nasim_prs/result/extracted_snp_onco_temp.txt")
  qctool.command[i,1] <- temp
  
}

write.table(qctool.command,file = paste0("./risk_prediction/Nasim_prs/code/qc_extract_snp_onco_temp.sh"),col.names = F,row.names = F,quote=F)
