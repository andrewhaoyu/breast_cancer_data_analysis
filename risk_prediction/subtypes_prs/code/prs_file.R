...
for(i in 1:n.pthres){
  for(j in 1:length(select.names)){
    ...
    colnames(prs) <- c("SNP","effect_allele","beta")
    dim(prs)
    write.table(prs,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/subtypes_prs/result/",select.names[j],"_prs_pvaluecut_",i,"_121019.file"),row.names=F,col.names=T,quote=F)
