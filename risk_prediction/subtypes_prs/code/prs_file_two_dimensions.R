...
for(i in 1:n.pthres){
  for(k in 1:n.pthres){
    ...
    dim(prs)
    write.table(prs,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/subtypes_prs/result/",select.names[j],"_prs_pvaluecut_",i,"_",k,"_121019.file"),row.names=F,col.names=T,quote=F)
