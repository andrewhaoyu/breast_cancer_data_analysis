...
total <- length(select.names)*n.pthres*n.pthres
auc <- rep(0,total)
auc95 <- rep("c",total)
method <- rep("c",total)
p1 <- rep(0,total)
...
for(l in 1:length(names.subtypes)){
  ...
  auc.result.temp = auc.result[ldx,]
  kdx <- which.max(auc.result.temp$auc)
  best.result = rbind(best.result,auc.result.temp[kdx,])
}
...
for(j in 1:length(select.names)){
  ...
  i <- which(pthres==auc.result.sub$p1[idx])
  k <- which(pthres==auc.result.sub$p2[idx])
      ...
      temp.out <- system(code,intern = T)
      n.snp[ind] <- as.numeric(gsub(paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/subtypes_prs/result/",select.names[j],"_prs_pvaluecut_",i,"_",k,"_121019.file"),"",temp.out))
...
auc.vad <- data.frame(auc,
                         ...
                         subtypes,
                         n.snp)
write.csv(auc.vad,file = "/data/zhangh24/breast_cancer_data_analysis/risk_prediction/LD_clumping/result/auc.result.vad.2d.csv")
