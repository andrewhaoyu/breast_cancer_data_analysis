...
for(i in 1:22){
  convert_dosage_code <- paste0("zcat /data/zhangh24/BCAC/impute_onco/chr",i,".gz | awk -v C=2 -v TF=/data/zhangh24/BCAC/impute_plink_onco/clump_snp_121019 -f /home/zhangh24/bin/Element.Matching.awk  | awk -v chr=",i," '{printf chr\"\\t\"$2\"\\t\"$3\"\\t\"$4\"\\t\"$5; for(i=6; i<NF; i+=3) {if($(i+0) == 0 && $(i+1) == 0 && $(i+2) == 0) printf \"\\tNA\"; else printf \"\\t\"$(i+0)*2+$(i+1)*1+$(i+2)*0}; printf \"\\n\"}' | gzip > /data/zhangh24/BCAC/impute_onco_dosage/dosage_chr_new_try_121019",i)
  convert_dosage[i,1] <- convert_dosage_code
}
#write out the command and submit use cluster
write.table(convert_dosage,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/LD_clumping/code/convert.dosage.new.try.sh_121019"),col.names = F,row.names = F,quote=F)


#merge to one dosage file
merge.code <- "cat "
for(i in 1:22){
  merge.code <- paste0(merge.code," /data/zhangh24/BCAC/impute_onco_dosage/dosage_chr_new_try_121019",i)
}
merge.code <- paste0(merge.code, "> /data/zhangh24/BCAC/impute_plink_onco/dosage_all_new_try_121019")
#write out the command and submit use cluster
write.table(merge.code,file = paste0("/data/zhangh24/breast_cancer_data_analysis/risk_prediction/LD_clumping/code/merge.chr.dosage_new_try_121019.sh"),col.names = F,row.names = F,quote=F)
