...
sd_meta= rep(0,n)
ERNeg_control_meta_result_final = data.frame(rs_id,CHR,position,logodds_meta,sd_meta,stringsAsFactors=F)
...
for(i in 1:1000){
  ...
  temp = nrow(ERNeg_control_meta_result)
  ERNeg_control_meta_result_final[(1+total.num):(temp+total.num),]=ERNeg_control_meta_result
