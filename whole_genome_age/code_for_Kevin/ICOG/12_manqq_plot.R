#generate mahattan plot
setwd("/data/NC_BW/HZ_SF/Figures/")
source("/data/NC_BW/HZ_SF/theme_publication.R")
library(data.table)
library(qqman)
library(dplyr)
#data
load("/data/NC_BW/HZ_SF/data3.rdata")
#filter SNPs with low imputation quality( info > 0.3)
dat = data_select %>% 
  filter(info > 0.3) %>% 
  mutate(MAF = ifelse(exp_freq_a1<=0.5,exp_freq_a1,1-exp_freq_a1)) %>% 
  select(var_name, CHR, position, p.acat, MAF) %>% 
  rename(P = p.acat, 
         BP = position)
idx_order = order(dat$CHR, dat$BP)
dat = dat[idx_order,]
# dat = dat %>% 
#   mutate(CHR = ifelse(CHR==23, "X", CHR))
#effective sample size
N.effect <- (106278*91477)/(10628+91477)
library(readr)
library(dplyr)
x = dat$P
z = qnorm(x / 2)
lambda = round(median(z^2) / qchisq(0.5,1), 3)
#convert lambda to lambda_1000
...
dat$BPcum <- NA
s <- 0
s_start = min(dat[dat$CHR == 1,]$BP)
nbp <- c()
for (i in unique(dat$CHR)){
  nbp[i] <- max(dat[dat$CHR == i,]$BP)
  dat$BPcum[dat$CHR == i] <- dat$BP[dat$CHR == i] + s - s_start + 100
  s <- s + nbp[i]
}
library(dplyr)
axis.set <- dat %>% 
...
#x-axis is too crowded; make select sub
axis.set = axis.set %>% 
 mutate(CHR_label = ifelse(CHR==23, "X", CHR)) %>% 
  filter(CHR_label %in% c("18", "20", "22")==F)
...
#          label= "C2") +
scale_x_continuous(expand = c(0.01,0), label = axis.set$CHR_label, breaks = axis.set$center) +
  ...
  labs(x = NULL, 
       y = "-log10(p)", 
       linetype = "",
       title = paste0("Breast Cancer Subtypes GWAS results"))+
  ...
  outpath <-"/data/NC_BW/HZ_SF/Figures/"  


ggsave(filename=paste0("man_bc_original.png"),
...
png(filename = paste0(outpath,"/QQ_BC_GWAS.png"), width = 8, height = 8, units = "in",res=300)
...
title(paste0("QQ-plot for Breast Cancer GWAS"))
dev.off()
