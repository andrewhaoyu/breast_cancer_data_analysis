...
PowerCompare <- function(y.pheno.mis,G,x_covar){
  ...
  score.test.support.fixed <- ScoreTestSupportMixedModel(
    y.pheno.mis,
    baselineonly = as.matrix(as.numeric(G)),
...
thetas <- read.csv(file = 'theta_set.csv')
theta_test3 = as.numeric(log(thetas[6,4:14]))*0.6
...
#####for hetergeneity test: only consider s ==2 | s==3
#####two scenaiors
...
#p_topo
p_ftop <- matrix(0,s_times,sc*n.sizes)
...
for(i in 1:s_times){
  ...
  for(s in 1:sc){
    if(s==1){
      ...
      # theta_test <- c(0,0.06,rnorm(9,0,0.02))
    ...
    for(n in sizes){
      ...
      #set up p-value cutoff as 5E-08###############
      #put the replicate as 1 if p <=5E-08, 0 if p> 5E-08
      #power = sum(number of 1)/number of replicates
...
result <- list(p_mglobal_result,p_ftop,p_topo)

#change the output directory to your own directory
save(result,file=paste0("./result_four/simu_result_0.25_",i1,".Rdata"))
