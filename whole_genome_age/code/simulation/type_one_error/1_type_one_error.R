...
#generate the second stage effect for genotype
theta_test <- rep(0,5)
#generate teh second stage effect for covariates
...
#simulated times as 100,000
  ...
  p_global_result <- matrix(0, s_times, length(size))
  temp <- 1  
  for(k in 1:length(sizes)){
    ...
    for(i in 1:s_times){
      ...
      p_global_result[i,k] <- as.numeric(model.result[[1]])
      #######################################################
