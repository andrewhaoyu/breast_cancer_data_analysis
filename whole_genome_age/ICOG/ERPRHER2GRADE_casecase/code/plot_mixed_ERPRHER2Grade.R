...
if(i1==1){
  ...
  # idx <- which(meta_result_shared_1p$CHR==18&meta_result_shared_1p$position==10354649)
  ...
  qq(gwas_result$P,main=paste0(" Global Test Association Manhattan QQ Plot"),xlim=c(0,7),ylim=c(0,20))
  dev.off()
 ...
 load("/spin1/users/zhangh24/breast_cancer_data_analysis/whole_genome_age/ICOG/ERPRHER2GRADE_fixed_baseline/result/extract.list.ld")
  ...
  idx.cut <- which((gwas_result$BP%in%extract.list.ld$position)&(gwas_result$CHR%in%extract.list.ld$CHR))
  gwas_result <- gwas_result[-idx.cut,]
  png(paste0("./result/plot/man_filter_Ju.png"),width = 7.635,height =4.7175,units = "in",res = 600)
  manhattan(gwas_result,suggestiveline = F, cex.axis = 2,main=paste0(" Global Test Association Manhattan Plot"),ylim=c(0,15))
  dev.off()
