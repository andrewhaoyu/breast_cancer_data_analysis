...
# subtypes estimates used complete data
#-------------------------------------------------------------------
#---------------------------------------#---------------------------------------
...
standard_result <- standard_result %>% 
  mutate(chr.pos = paste0(CHR,":",Position.Onco))
...
p.value.known <- known_snp_subtypes$p_overall

idx.control <- which(data2$Behaviour1==0)

LD2 <- cor(x.test.all.mis2[idx.control,])^2
known.snp.infor.p <- known_snp_subtypes %>% 
  select(Best.published.SNP,CHR.x,position.x,p_overall)
...
known_snp_standard_cal = known_snp_subtypes %>% 
  select(beta_overall,var_overall,EAFcontrols.Onco)
...
colnames(known_snp_subtypes)[104] <- "beta_CIMBA"
idx.known <- grep("beta_",colnames(known_snp_subtypes))
jdx.known <- grep("var_",colnames(known_snp_subtypes))
zdx.known <- grep("MAF",colnames(known_snp_subtypes))
colnames(discovery_snp_subtypes)[109] <- "beta_CIMBA"
idx.dis<- grep("beta_",colnames(discovery_snp_subtypes))
jdx.dis <- grep("var_",colnames(discovery_snp_subtypes))
zdx.dis <- grep("exp_freq_a1",colnames(discovery_snp_subtypes))[1]
