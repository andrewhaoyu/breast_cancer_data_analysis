...
LDP <- function(y,beta.train,beta.test,beta.vad,p.train,
                p.thr,pop.ind){
  y = y[[pop.ind]]
  n.sub <- length(y)
  n.train <- n.sub*10/12
  ...
  for(k in 1:length(p.thr)){
    ...
    if(length(idx)==0){
      ...
      model1 <- lm(y.test~prs.test-1)
      r2.test[k] <- summary(model1)$adj.r.squared
      model2 <- lm(y.vad~prs.vad-1)
...
p.thr <- c(10^-8,10^-7,10^-6,10^-5,10^-4,10^-3,10^-2,0.1,
           0.3,0.5)
