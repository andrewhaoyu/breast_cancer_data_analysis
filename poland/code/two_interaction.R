#Goal: run the score test with two way interaction 
rm(list=ls())
...
#Generate Pairwise interaction design matrix
y <- y.pheno.mis2
...
y.pheno.complete <- y[-missing.data.vec,,drop=F]
  ...
  z.design.pairwise.interaction <- GenerateZDesignPairwiseInteraction(tumor.character.cat,
                                                                      ...
                                                                      freq.subtypes,
                                                                      cutoff=10)
z.design.test <- z.design.pairwise.interaction[,3:ncol(z.design.pairwise.interaction)]
Onc_ID <- data2$Onc_ID

rm(data2)
gc()
load("./poland/result/whole_genome/score.test.support.onco.ERPRHER2Grade_pairwise.Rdata")
