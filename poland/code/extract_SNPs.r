...
for(i in 1:567){
  geno.file <- Files[i]
  temp <- paste0("/spin1/users/zhangh24/qctool_v1.4-linux-x86_64/qctool -g ",Files[i]," -incl-rsids /spin1/users/zhangh24/breast_cancer_data_analysis/poland/result/whole_genome/extract_id_onco.txt -og /spin1/users/zhangh24/breast_cancer_data_analysis/poland/result/whole_genome/top_signal_extract",i,".txt")
  qctool.command[i,1] <- temp
  
}


write.table(qctool.command,file = paste0("/spin1/users/zhangh24/breast_cancer_data_analysis/poland/code/qc_extract_snp.sh"),col.names = F,row.names = F,quote=F)
