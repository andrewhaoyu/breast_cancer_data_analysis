#goal: sensitivity analysis with grade effect fixed
rm(list=ls())
...
z.design.support <- cbind(1,z.standard[,c(1,4)])
z.design.test <- z.standard[,2:3]
Onc_ID <- data2$Onc_ID
...
#original delta0 is designed for baseline, ER as fixed
#this version we add grade as fixed
delta0 = c(delta0[1:17],c(0,0,0),delta0[20:length(delta0)])
no.cores <- 2
library(foreach)
library(doParallel)
...
result.list <- foreach(job.i = 1:2)%dopar%{
  ...
  score_result2 <- matrix(0,inner.file.num,num.of.tumor-2)
  infor_result2 <- matrix(0,inner.file.num,(num.of.tumor-2)^2)
  ...
  close(con)
  result <- list(snpid_result,freq.all,
...
score_result2 <- matrix(0.1,file.num,num.of.tumor-2)
infor_result2 <- matrix(0.1,file.num,(num.of.tumor-2)^2)
...
for(i in 1:inner.size){
  ...
  freq.all[total+(1:temp)] <- result.temp[[2]]
   score_result2[total+(1:temp),] <- result.temp[[3]]
   infor_result2[total+(1:temp),] <- result.temp[[4]]
  # 
  total <- temp+total
}

result <- list(snpid_reuslt=snpid_result,freq.all=freq.all,
               score_result2=score_result2,
               infor_result2=infor_result2)

save(result,file=paste0("./poland/result/whole_genome/grade_fixed_",i1,"_",i2))
