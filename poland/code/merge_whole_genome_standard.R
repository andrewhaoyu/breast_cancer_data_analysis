...
p_value1 <- rep(0,num.total)
p_value2 <- rep(0,num.total)
# number.of.tumor <- 4
# score <- matrix(0,nrow=num,ncol = (number.of.tumor+1))
# infor <- matrix(0,nrow = num,ncol = (number.of.tumor+1)^2)
# score2 <- matrix(0,nrow=num,ncol = (number.of.tumor-1))
# infor2 <- matrix(0,nrow = num,ncol = (number.of.tumor-1)^2)
...
for(i in 1:567){
  ...
  for (k in 1:5) {
    #print(k)
    load(
      paste0("./poland/result/whole_genome/standard_analysis",idx[i],"_",k)
    )
    
    temp <- length(result[[1]])
    rs_id[num.total+(1:temp)] <- result[[1]]
    p_value1[num.total+(1:temp)] <- result[[2]]
    p_value2[num.total+(1:temp)] <- result[[3]]
...
onco_result_standard <- data.frame(onco_info,p_value1,p_value2,CHR)
#onco_result_casecase <- data.frame(onco_info,score2,infor2,CHR)


# save(onco_result_fixed,file="/data/zhangh24/breast_cancer_data_analysis/poland/result/whole_genome/onco_result_fixed.Rdata")
# save(onco_result_casecase,file="/data/zhangh24/breast_cancer_data_analysis/poland/result/whole_genome/onco_result_casecase.Rdata")

idx <- which(onco_result_standard$exp_freq_a1>=0.05&
               onco_result_standard$exp_freq_a1<=0.95)
onco_result_standard_5p <- onco_result_standard[idx,]
#onco_result_casecase_5p <- onco_result_casecase[idx,]
save(onco_result_standard_5p,file="/data/zhangh24/breast_cancer_data_analysis/poland/result/whole_genome/onco_result_standard_5p.Rdata")
