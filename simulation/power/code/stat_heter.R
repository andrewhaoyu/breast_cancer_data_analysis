...
###########simulate results for MTOP, FTOP, complete data two-stage model with only complete data global heterogeneity test result
...
PowerCompare <- function(y.pheno.mis,G,x_covar,theta_intercept,theta_test,theta_covar){
  ...
  fixed.result <- DisplaySecondStageTestResult(odds,sigma)
  p_global_complete <- fixed.result[12]
...
result.list <- foreach(job.i = 1:2)%dopar%{
  ...
  heter_pattern = 2
  p_global_result <- rep(0,heter_pattern*n.sizes*s_times)
  p_mglobal_result <- rep(0,heter_pattern*n.sizes*s_times)
  p_global_complete <- rep(0,heter_pattern*n.sizes*s_times)
...
stopImplicitCluster()
save(result.list,file=paste0("./simulation/power/result/heter_result_",i1,".Rdata"))
